...
public abstract class BluetoothHeadsetUtils {
	...
	/**
	 * Should call this on onResume or onDestroy. Unregister broadcast
	...
	/**
	 * Register for bluetooth headset connection states and Sco audio
	 * states. Try to connect to bluetooth headset audio by calling
	 * startBluetoothSco(). This is a work around for API < 11 to detect if
	 * a headset is connected before the application starts.
	 * 
	 ...
	 * audio to/from a bluetooth SCO headset while the phone is not in
	 * call."
	 * 
	 ...
	 * mBroadcastReceiver will receive an ACTION_SCO_AUDIO_STATE_CHANGED
	 * with intent extra SCO_AUDIO_STATE_CONNECTED, then we assume that a
	 * headset is connected.
	 * 
	 * @return false if device does not support bluetooth or current
	 *         platform does not supports use of SCO for off call.
	 */
	...
	private boolean startBluetooth() {
		...
		if (mBluetoothAdapter != null) {
			if (mAudioManager.isBluetoothScoAvailableOffCall()) {
				...
				// Need to set audio mode to MODE_IN_CALL for
				// call to startBluetoothSco() to succeed.
				mAudioManager.setMode(AudioManager.MODE_IN_CALL);

				mIsCountDownOn = true;
				// mCountDown repeatedly tries to start
				// bluetooth Sco audio connection.
				mCountDown.start();
	 ...
	 * 
	 * @return false if device does not support bluetooth or current
	 *         platform does not supports use of SCO for off call or error
	 *         in getting profile proxy.
	 */
	...
	private boolean startBluetooth11() {
		...
		if (mBluetoothAdapter != null) {
			if (mAudioManager.isBluetoothScoAvailableOffCall()) {
				// All the detection and audio connection are
				// done in mHeadsetProfileListener
				if (mBluetoothAdapter.getProfileProxy(mContext, new BluetoothProfile.ServiceListener() {
					...
					public void onServiceConnected(int profile, BluetoothProfile proxy) {
						...
						// If a headset is connected before this application
						// starts,
						// ACTION_CONNECTION_STATE_CHANGED will not be
						// broadcast.
						...
						if (devices.size() > 0) {
							...
							// Should not need count down timer, but just in
							// case.
							// See comment below in
							// mHeadsetBroadcastReceiver onReceive()
							mIsCountDownOn = true;
							mCountDown11.start();
						...
						// During the active life time of the app, a user may
	...
	/**
	 * API >= 11 Unregister broadcast receivers and stop Sco audio
	 * connection and cancel count down.
	 */
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	protected void stopBluetooth11() {
		...
		if (mBluetoothHeadset != null) {
			...
			// change orientation or close with headset still turns
			// on.
	...
	/**
	 * Broadcast receiver for API < 11 Handle headset and Sco audio
	 * connection states.
	 */
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(BluetoothDevice.ACTION_ACL_CONNECTED)) {
				...
				if (bluetoothClass != null) {
					// Check if device is a headset. Besides
					// the 2 below, are there other
					// device classes also qualified as
					// headset?
					int deviceClass = bluetoothClass.getDeviceClass();
					if (deviceClass == BluetoothClass.Device.AUDIO_VIDEO_HANDSFREE
							|| deviceClass == BluetoothClass.Device.AUDIO_VIDEO_WEARABLE_HEADSET) {
						// start bluetooth Sco audio
						// connection.
						// Calling startBluetoothSco()
						// always returns faIL here,
						// that why a count down timer
						// is implemented to call
						// startBluetoothSco() in the
						// onTick.
						mAudioManager.setMode(AudioManager.MODE_IN_CALL);
						mIsCountDownOn = true;
						mCountDown.start();
				...
				if (state == AudioManager.SCO_AUDIO_STATE_CONNECTED) {
					...
					// thing when Sco audio is connected.
					onScoAudioConnected();
					...
					// Always receive
					// SCO_AUDIO_STATE_DISCONNECTED on call
					// to startBluetooth()
					// which at that stage we do not want to
					// do anything. Thus the if condition.
					if (!mIsStarting) {
						mIsOnHeadsetSco = false;

						// Need to call
						// stopBluetoothSco(), otherwise
						// startBluetoothSco()
						// will not be successful.
						mAudioManager.stopBluetoothSco();

						// override this if you want to
						// do other thing when Sco audio
						// is disconnected.
	...
	private CountDownTimer mCountDown = new CountDownTimer(10000, 1000) {
		...
		public void onTick(long millisUntilFinished) {
			// When this call is successful, this count down timer
			// will be canceled.
		...
		public void onFinish() {
			...
			// successful.
			// Should implement something to inform user of this
			// failure
			mIsCountDownOn = false;
			mAudioManager.setMode(AudioManager.MODE_NORMAL);
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED)) {
				...
				if (state == BluetoothHeadset.STATE_CONNECTED) {
					...
					// Calling startVoiceRecognition always
					// returns false here,
					// that why a count down timer is
					// implemented to call
					...
					// override this if you want to do other
					// thing when the device is connected.
					onHeadsetConnected();

					Log.d(TAG, "Start count down"); //$NON-NLS-1$
				} else if (state == BluetoothHeadset.STATE_DISCONNECTED) {
					// Calling stopVoiceRecognition always
					// returns false here
					// as it should since the headset is no
					// longer connected.
					if (mIsCountDownOn) {
						mIsCountDownOn = false;
						mCountDown11.cancel();
			...
			} else // audio
			{
				...
				if (state == BluetoothHeadset.STATE_AUDIO_CONNECTED) {
					...
					// override this if you want to do other
					// thing when headset audio is
					// connected.
					onScoAudioConnected();
				} else if (state == BluetoothHeadset.STATE_AUDIO_DISCONNECTED) {
					mIsOnHeadsetSco = false;

					// The headset audio is disconnected,
					// but calling
					// stopVoiceRecognition always returns
					// true here.
					mBluetoothHeadset.stopVoiceRecognition(mConnectedHeadset);
	...
	private CountDownTimer mCountDown11 = new CountDownTimer(10000, 1000) {
		...
		@Override
		public void onTick(long millisUntilFinished) {
			// First stick calls always returns false. The second
			// stick
			// always returns true if the countDownInterval is set
			// to 1000.
